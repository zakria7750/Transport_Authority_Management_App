...
function BreakdownDetailSheet({
  ...
  const timeline = [
    trip?.completedAt
      ? `تأكيد خروج النهمة — ${trip.completedAt}`
      : "تأكيد خروج النهمة",
    `تسجيل العطل — ${breakdown.location}`,
    breakdown.breakdownPlace
      ? `تحديد مكان العطل — ${breakdown.breakdownPlace}`
      : null,
    breakdown.action
      ? `اختيار مصير النهمة — ${actionLabel(breakdown.action)}`
      : null,
    ...
    breakdown.action === "إبقاء_النهمة"
      ? "إعادة تسجيل المالك في آخر الكشف"
      : "استعادة حالة المالك السابقة",
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 160 }}>
      ...
      >
        ...
        >
          <div>
            <h3 style={{ margin: 0, color: th.text, fontSize: 17 }}>
              تفاصيل العطل
            </h3>
            <p style={{ margin: "4px 0 0", color: th.sub, fontSize: 11 }}>
              {breakdown.driverName} · {breakdown.plate}
            </p>
          </div>
          <button
            type="button"
        ...
        <section style={{ marginBottom: 16 }}>
          <h4 style={{ margin: "0 0 8px", fontSize: 12, color: T.primary }}>
            بيانات المعطل
          </h4>
          <div
            style={{ background: th.inputBg, borderRadius: 12, padding: 12 }}
          >
            ...
            <InfoRow
              label="الحمولة"
              value={breakdown.payload ?? trip?.payload}
            />
            <InfoRow
              label="الوجهة"
              value={breakdown.destination ?? trip?.destination}
            />
            <InfoRow
              label="رقم الفك"
              value={trip?.breakNum ?? breakdown.breakNum}
            />
          </div>
        </section>

        <section style={{ marginBottom: 16 }}>
          <h4 style={{ margin: "0 0 8px", fontSize: 12, color: T.warning }}>
            بيانات العطل
          </h4>
          <div
            style={{ background: th.inputBg, borderRadius: 12, padding: 12 }}
          >
            ...
            <InfoRow
              label="مصير النهمة"
              value={actionLabel(breakdown.action)}
            />
            <InfoRow label="الملاحظات" value={breakdown.notes} />
            <InfoRow
              label="الحالة"
              value={breakdown.status === "نشط" ? "جارٍ" : "منتهٍ"}
            />
          </div>
        </section>

        <section style={{ marginBottom: 16 }}>
          <h4 style={{ margin: "0 0 8px", fontSize: 12, color: T.success }}>
            بيانات المسعف
          </h4>
          <div
            style={{ background: th.inputBg, borderRadius: 12, padding: 12 }}
          >
            <InfoRow
              label="الاسم"
              value={breakdown.rescuerName ?? rescuer?.ownerName}
            />
            <InfoRow label="اللوحة" value={rescuer?.plate} />
            <InfoRow label="نوع النهمة" value={breakdown.rescuerTripType} />
            <InfoRow
              label="رقم الفك"
              value={breakdown.breakNum ?? rescuerTrip?.breakNum}
            />
            {breakdown.location === "بعيد" && (
              <InfoRow
                label="التعويضات"
                value={breakdown.compensationGiven ?? 0}
              />
            )}
          </div>
        </section>
        ...
        <section>
          <h4 style={{ margin: "0 0 10px", fontSize: 12, color: th.text }}>
            سجل العمليات
          </h4>
...
export function BreakdownsScreen() {
  ...
  const allItems = useMemo((): ListItem[] => {
    ...
    for (const breakdown of state.breakdowns) {
      const trip = breakdown.tripId
        ? state.trips.find((t) => t.id === breakdown.tripId)
        : undefined
  ...
  const filteredItems = useMemo(() => {
    ...
    if (tab === "مكتملة")
      items = items.filter((item) => item.kind === "completed")
    if (tab === "قريب")
      items = items.filter(
        (item) =>
          item.kind === "breakdown" && item.breakdown.location === "قريب",
      )
    if (tab === "بعيد")
      items = items.filter(
        (item) =>
          item.kind === "breakdown" && item.breakdown.location === "بعيد",
      )
    ...
    return items.filter((item) => {
      if (item.kind === "breakdown") {
        return matchesNameOrPlate(
          q,
          item.breakdown.driverName,
          item.breakdown.plate,
        )
      }
        ...
        <div style={{ padding: "14px 16px" }}>
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <InfoRow
                label="الوجهة"
                value={trip.destination || trip.province}
              />
              <InfoRow label="رقم الفك" value={trip.breakNum} />
              <InfoRow
                label="تاريخ الخروج"
                value={trip.completedAt ?? trip.createdAt}
              />
            </div>
            <span
              style={{
  ...
  const renderCompletedCard = (
    item: Extract<ListItem, { kind: "completed" }>,
  ) => {
    const { trip, driver } = item
    return (
  ...
  const renderBreakdownCard = (
    item: Extract<ListItem, { kind: "breakdown" }>,
  ) => {
        ...
        <div style={{ padding: "14px 16px" }}>
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <InfoRow
                label="نوع البابور"
                value={driver?.type ?? breakdown.tripType}
              />
              <InfoRow
                label="الحمولة"
                value={breakdown.payload ?? trip?.payload}
              ...
              <InfoRow
                label="نوع نهمة المسعف"
                value={breakdown.rescuerTripType}
              />
              {!isClose && (
                <InfoRow
                  label="عدد التعويضات"
            ...
            >
              {isClose ? "قريب" : "بعيد"} ·{" "}
              {breakdown.status === "نشط" ? "جارٍ" : "منتهٍ"}
            </span>
          </div>
    ...
    >
      ...
      >
        {[
          ...
          >
            <strong
              style={{ display: "block", color: color as string, fontSize: 18 }}
            >
              {value as number}
            </strong>
          ...
          {TAB_LABELS.map(({ key, label }) => (
            <button
              ...
              style={{
                ...
                whiteSpace: "nowrap",
                background:
                  tab === key ? T.primary : th.dark ? "#1E2D40" : "#F1F5F9",
      ...
      {detailBreakdown && (
        <BreakdownDetailSheet
          ...
          trip={
            detailBreakdown.tripId
              ? state.trips.find((t) => t.id === detailBreakdown.tripId)
              : undefined
          }
      ...
      {confirmDelete && (
        <div style={{ position: "absolute", inset: 0, zIndex: 170 }}>
          ...
          >
            <h3 style={{ margin: "0 0 8px", color: th.text, fontSize: 16 }}>
              حذف العطل؟
            </h3>
            <p
              style={{
            ...
            >
              سيتم التراجع عن جميع العمليات المرتبطة: حالة المالك، نهمة المسعف،
              التعويضات، والتسلسل.
            </p>
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button
                type="button"
                onClick={() => confirmDeleteBreakdown(confirmDelete)}
                style={{
                  flex: 1,
                  padding: 12,
                  borderRadius: 10,
