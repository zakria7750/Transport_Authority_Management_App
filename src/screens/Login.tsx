...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    setLoading(false)
    if (!user) {
      setError("خطأ في البيانات — تحقق من اسم المستخدم وكلمة المرور والدور")
      return
    }
    dispatch({ type: "LOGIN", user })
  }

  const biometricUser = state.users[0]
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          >
            ...
            <option value="موظف_تسجيل" style={{ background: "#0F2040" }}>موظف تسجيل</option>
          </select>
        </div>
        ...
        <div style={{ fontSize: 11, color: "#475569", textAlign: "center", lineHeight: 1.8 }}>
          ...
          <div>موظف تسجيل: <span style={{ color: "#64748B" }}>تسجيل1 / reg</span></div>
        </div>
      </div>

      {state.biometricEnabled && biometricUser && (
        <button
          onClick={() => dispatch({ type: "LOGIN", user: biometricUser })}
          style={{
            marginTop: 24,
            background: "none",
